...
def read_wave(wav_file):
    ...
    fs, signal = wavfile.read(abs_path)
    return signal, fs
...
def analyze_pred_vs_actual(args):
    ...
    output_wav = os.path.join('lightning_logs', 'version_1', 'checkpoints', 'y_test.wav')
    pred_wav = os.path.join('lightning_logs', 'version_1', 'checkpoints', 'y_pred.wav')
    input_wav = os.path.join('lightning_logs', 'version_1', 'checkpoints', 'x_test.wav')
