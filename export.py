    ...
    for i in range(-1, len(dilations) + 1):
        ...
        if i == -1:
            data_out["variables"].append(
                {
                    "layer_idx": i,
                    "data": [
                        str(w) for w in (sd["wavenet.input_layer.weight"]).permute(a, b, c).flatten().cpu().numpy().tolist()
                    ],
                    "name": "W",
                }
            )
            data_out["variables"].append(
                {
                    "layer_idx": i,
                    "data": [str(b) for b in (sd["wavenet.input_layer.bias"]).flatten().cpu().numpy().tolist()],
                    "name": "b",
                }
            )
        # Linear Mix Layer
            ...
            data_out["variables"].append(
                {
                    "layer_idx": i,
                    "data": [
                        str(w) for w in (sd["wavenet.linear_mix.weight"]).permute(a, b, c).flatten().cpu().numpy().tolist()
                    ],
                    "name": "W",
                }
            )
            ...
            data_out["variables"].append(
                {
                    "layer_idx": i,
                    "data": [str(b) for b in (sd["wavenet.linear_mix.bias"]).cpu().numpy().tolist()],
                    "name": "b",
                }
            )
        # Hidden Layers
            ...
            data_out["variables"].append(
                {
                    ...
                    "data": [
                        str(w)
                        for w in sd["wavenet.hidden." + str(i) + ".weight"].permute(a, b, c).flatten().cpu().numpy().tolist()
                    ],
                    "name": "W_conv",
                }
            )
            data_out["variables"].append(
                {
                    "layer_idx": i,
                    "data": [str(b) for b in sd["wavenet.hidden." + str(i) + ".bias"].flatten().cpu().numpy().tolist()],
                    "name": "b_conv",
                }
            )
            data_out["variables"].append(
            ...
            data_out["variables"].append(
                {
                    "layer_idx": i,
                    "data": [str(b2) for b2 in sd["wavenet.residuals." + str(i) + ".bias"].flatten().cpu().numpy().tolist()],
                    "name": "b_out",
                }
            )
